...
|---|---|---|
| Link | Postcard | Unfurl image, scalloped-feel stamp (`accent`), VISIT pill, favicon + site in meta voice |
...
| Emoji | Die-cut sticker | Oversized glyph, drop shadow; silent — no back, no thread, no reactions; future: thick white die-cut border (FRIDGE pattern) |
...
- `ios app` / `ios signup welcome` — quiet mobile canvas; one-CTA sheet *(shipped: social-first auth)*
- `organize shared items 1–2` — collections as shelves/folders *(concept seed: space items view)*
- `scroll timeline 1–2` — tick-rail scrollbar *(concept seed: timeline navigation)*
- `stickers idea (border)` — die-cut white borders *(concept seed: sticker treatment)*
